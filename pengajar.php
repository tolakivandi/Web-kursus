 ...
 <body class="bg-gradient-to-r from-green-200 to-green-400">
     ...
     <div class="flex flex-col items-center justify-center h-screen bg-green-900">
         <table class="shadow-2xl font-[poppins] border-2 border-green-200 w-9/12 py-3 px-6">
             ...
             <?php foreach ($pengajar as $pjr) : ?>
                 <tr class="duration-300 cursor-pointer bg-green-300">
                     ...
                     <td class="px-6 py-3 font-semibold"> <?= $pjr['nama_pengajar']; ?> </td>
                     <td class="px-6 py-3 font-semibold"> <?= $pjr['alamat_pengajar']; ?> </td>
                     <td class="px-6 py-3 font-semibold"> <?= $pjr['no_telp_pengajar']; ?> </td>
                     <td class="px-6 py-3 font-semibold"> <?= $pjr['email_pengajar']; ?> </td>
                     <td class="flex justify-center gap-5 px-6 py-3">
                         <a href="./pengajar/updatepengajar.php?id=<?= $pjr["id_pengajar"] ?>" class="px-4 py-3 font-semibold text-black border-2 border-black bg-green-700 rounded-xl">Update</a> |
                         <a href="./pengajar/deletepengajar.php?id=<?= $pjr["id_pengajar"] ?>" onclick="return confirm('yakin?');" class="px-4 py-3 font-semibold text-black border-2 border-black bg-green-700 rounded-xl">Delete</a>
                     </td>
                 </tr>
                 <?php $i++; ?>
             <?php endforeach; ?>
         ...
         <button type="text" class=" bg-green-800 hover:bg-indigo-400 focus:bg-indigo-400 text-white font-semibold rounded-lg px-4 py-3 mt-6 ml-[925px] border-2 " style="box-shadow: 7px 14px 23px 0px rgba(0,0,0,0.75);
-webkit-box-shadow: 7px 14px 23px 0px rgba(0,0,0,0.75);
-moz-box-shadow: 7px 14px 23px 0px rgba(0,0,0,0.75);"> <a href="./pengajar/createpengajar.php">Tambah Data pengajar</a> </button>
     </div>
 </body>
 ...
 <body class="bg-gradient-to-r from-green-200 to-green-400">
   ...
   <div class="flex flex-col items-center justify-center h-screen bg-green-800 ">
     <table class="shadow-2xl font-[Poppins] border-2 border-cyan-200 w-8/12" style="box-shadow: 7px 14px 23px 0px rgba(0,0,0,0.75);
...
-moz-box-shadow: 7px 14px 23px 0px rgba(0,0,0,0.75);">
       <thead class="text-white">
         <tr>
           ...
           <th class="py-3 bg-green-800">Nama Peserta</th>
           <th class="py-3 bg-green-800">Alamat Peserta</th>
           <th class="py-3 bg-green-800">No Telepon Peserta</th>
           <th class="py-3 bg-green-800">Email Peserta</th>
           <th class="py-3 bg-green-800">ACTION</th>
         </tr>
         <?php $i = 1; ?>
       ...
       <tbody class="text-center text-cyan-900">
         <tr class="duration-300 cursor-pointer bg-green-300">
           ...
           <td class="px-6 py-3 font-semibold"> <?= $item["nama_peserta"]; ?> </td>
           <td class="px-6 py-3 font-semibold"> <?= $item["alamat_peserta"]; ?> </td>
           <td class="px-6 py-3 font-semibold"> <?= $item["no_tlep_peserta"]; ?> </td>
           <td class="px-6 py-3 font-semibold"> <?= $item["email_peserta"]; ?> </td>
           <td class="flex justify-center gap-5 px-6 py-3">
             <a href="./peserta_kelas/updatepeserta_kel
             as.php?id=<?= $item["id_peserta"] ?>" class="px-4 py-3 font-semibold text-black border-2 border-black bg-green-700 rounded-xl">Update</a> |
             <a href="./peserta_kelas/deletepeserta_kelas.php?id=<?= $item["id_peserta"] ?>" onclick="return confirm('yakin?');" class="px-4 py-3 font-semibold text-black border-2 border-black bg-green-700 rounded-xl">Delete</a>
           </td>
         </tr>
         <?php $i++; ?>
       <?php endforeach; ?>
     ...
     <button type="text" class=" bg-green-800 hover:bg-indigo-400 focus:bg-indigo-400 text-white font-semibold rounded-lg px-4 py-3 mt-6 ml-[870px] border-2 " style="box-shadow: 7px 14px 23px 0px rgba(0,0,0,0.75);
-webkit-box-shadow: 7px 14px 23px 0px rgba(0,0,0,0.75);
-moz-box-shadow: 7px 14px 23px 0px rgba(0,0,0,0.75);"> <a href="./peserta_kelas/createpeserta_kelas.php">Tambah Data</a> </button>
   </div>
 </body>

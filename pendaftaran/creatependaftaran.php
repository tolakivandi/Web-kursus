 <?php
    ...
    $peserta = mysqli_query($connection, "SELECT * FROM peserta_kelas");
    $pengajar = mysqli_query($connection, "SELECT * FROM pengajar");
    $mahasiswa = mysqli_query($connection, "SELECT * FROM pendaftaran JOIN kelas ON pendaftaran.id_kelas = kelas.id_kelas 
    JOIN peserta_kelas ON pendaftaran.id_peserta = peserta_kelas.id_peserta JOIN pengajar ON pendaftaran.id_pengajar = pengajar.id_pengajar");
     ...
     <div class="flex items-center justify-center w-full h-screen overflow-hidden bg-white rounded-lg">
         <div class="w-1/3 md:flex">
             <div class="w-full px-6 py-8 md:p-8">
                 <h2 class="mb-2 text-2xl font-bold text-gray-700">Tambah Kelas</h2>
                 <form action="" method="POST">

                     <label for="countries" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select an option</label>
                     <select id="id_kelas" name="id_kelas" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-green-600 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                     ...
                     <select id="id_peserta" name="id_peserta" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-green-600 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                     ...
                     <select id="id_pengajar" name="id_pengajar" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-green-600 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                         <option selected>Pengajar</option>
                         <?php foreach ($pengajar as $item) : ?>
                             <option value="<?= $item['id_pengajar'] ?>"> <?= $item['nama_pengajar'] ?> </option>
                         <?php endforeach ?>
                     </select>

                     <div class="mb-4">
                         <label class="block mb-2 font-bold text-gray-700" for="tanggal_daftar">
                             tanggal_daftar
                         </label>
                         <input class="w-full px-3 py-2 leading-tight text-gray-700 border rounded appearance-none focus:outline-none focus:shadow-outline" id="tanggal_daftar" name="tanggal_daftar" type="text" placeholder="Masukkan tanggal_daftar">
                     </div>
                     <div class="mb-4">
                         <label class="block mb-2 font-bold text-gray-700" for="status_pembayaran">
                             status_pembayaran
                         </label>
                         <input class="w-full px-3 py-2 leading-tight text-black border rounded appearance-none focus:outline-none focus:shadow-outline" id="status_pembayaran" name="status_pembayaran" type="text" placeholder="Masukkan NO HP">
                     </div>

                     <div class="flex items-center justify-between">
                         <button class="px-4 py-2 font-bold text-white bg-green-800 rounded-lg hover:bg-blue-700 focus:outline-none focus:shadow-outline" type="submit" name="submit">
                             Daftar
                         </button>
                     </div>
                 </form>
